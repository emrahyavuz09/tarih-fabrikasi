...
# 1. PREMIUM UI ARCHITECTURE (CSS - TASARIM AYNI KALDI)
...
st.markdown("""
    ...
    div.stButton > button {
        ...
        padding: 12px 500px !important; font-weight: 700 !important; transition: 0.3s;
        width: 100%; max-width: 400px;
    }
    div.stButton > button:hover { transform: scale(1.02); box-shadow: 0 5px 15px rgba(255, 204, 0, 0.3); }

    .content-card {
        background-color: #1a1c23; padding: 30px; border-radius: 20px;
        border-left: 5px solid #FFCC00; margin: 30px auto; line-height: 1.8;
        font-size: 1.1rem; box-shadow: 0 10px 30px rgba(0,0,0,0.5);
    }

    .source-box { background: #252833; border-radius: 12px; padding: 15px; margin: 10px 0; border: 1px solid #3d4152; }
    .source-link { color: #FFCC00 !important; text-decoration: none; font-weight: 600; margin-right: 15px; }
    
    .loading-info { text-align: center; color: #FFCC00; font-style: italic; margin-top: 15px; }
    </style>
...
# 3. GÜNCEL PROMPT MANTIĞI (Özel Senaryolaştırma)
client = Groq(api_key="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")

SYSTEM_PROMPT = (
    "Sen akademik bir tarih profesörü ve sinematik bir hikaye anlatıcısısın. "
    ...
    "4. SENARYO ODAKLI PROMPTLAR: En sona '---PROMPTLAR---' yaz. Buraya, anlattığın hikayedeki EN ÖNEMLİ 10 ANI "
    "tıpkı şu örnekteki gibi görselleştir: 'Köle yolda yürürken ayağı takıldı ve altın kesesini buldu'. "
    "Promptlar hikayedeki spesifik olayları, karakterlerin hareketlerini ve o anki atmosferi tasvir etmeli. "
    "Tüm promptlar İngilizce, sinematik, 8k ve yüksek detaylı (Photorealistic) olmalıdır."
)
...
if st.button("ARAŞTIRMAYI BAŞLAT"):
    ...
    with st.spinner(""):
        st.markdown('<p class="loading-info">Arşivler taranıyor, sarsıcı gerçekler senaryolaştırılıyor...</p>', unsafe_allow_html=True)
